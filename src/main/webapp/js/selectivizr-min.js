...
(function (j) {
      ...
      for (var a = b.split(","), b = 0, e = a.length; b < e; b++) {
        ...
        a[b] = s.replace(G, function (a, b, c, d, e) {
          if (b) {
            if (l.length > 0) {
              ...
              if (f) {
                ...
                for (c = f.length; e < c; e++) {
                  for (var d = f[e], h = d.className, j = 0, m = a.length;
                    j < m; j++) {
                    var g = a[j];
                    if (!RegExp("(^|\\s)" + g.className + "(\\s|$)").test(
                        d.className) && g.b && (g.b === !0 || g.b(d)
                      === !0)) {
                      h = u(h, g.className, !0)
                    }
                  }
            ...
            if (b = c ? I(c) : !v || v.test(d) ? {className: w(d), b: !0}
                : null) {
  ...
  function r(a, c) {
    if (/^https?:\/\//i.test(a)) {
      ...
      == a.substring(0, a.indexOf("/", 8)) ? a : null;
    }
    if (a.charAt(0) == "/") {
      return c.substring(0, c.indexOf("/", 8)) + a;
    }
    var d = c.split(/[?#]/)[0];
    a.charAt(0) != "?" && d.charAt(d.length - 1) != "/" && (d = d.substring(0,
      d.lastIndexOf("/") + 1));
    return d + a
  }
  ...
  function y(a) {
    if (a) {
      return n.open("GET", a, !1), n.send(), (n.status == 200
  ...
  function U() {
    ...
    for (var d = a.length > 0 ? a[0].href : f.location.href, b = 0;
      b < f.styleSheets.length; b++) {
      if (c = f.styleSheets[b], c.href != i
        && (a = r(c.href, d))) {
        c.cssText = A(y(a));
      }
    }
    q.length > 0 && setInterval(function () {
      for (var a = 0, c = q.length; a < c; a++) {
        ...
        if (b.disabled !== b.a) {
          b.disabled
            ? (b.disabled = !1, b.a = !0, b.disabled = !0) : b.a = b.disabled
        }
      }
    }, 250)
  }
  ...
  if (!/*@cc_on!@*/true) {
    ...
    if (!(f.compatMode != "CSS1Compat" || m < 6 || m > 8 || !n)) {
      var z = {
        ...
        B = /:(:first-(?:line|letter))/g, C = /(^|})\s*([^\{]*?[\[:][^{]+)/g,
        G = /([ +~>])|(:[a-z-]+(?:\(.*?\)+)?)|(\[.*?\])/g,
        H = /(:not\()?:(hover|enabled|disabled|focus|checked|target|active|visited|first-line|first-letter)\)?/g,
        P = /[^\w-]/g, K = /^(INPUT|SELECT|TEXTAREA|BUTTON)$/,
        J = /^(checkbox|radio)$/, v = m > 6 ? /[\$\^*]=(['"])\1/ : null,
        E = /([(\[+~])\s+/g, F = /\s+([)\]+~])/g, Q = /\s+/g,
        x = /^\s*((?:[\S\s]*\S)?)\s*$/, i = "", o = " ", h = "$1";
      (function (a, c) {
        function d() {
        ...
        function b(d) {
          if (!(d.type == "readystatechange" && f.readyState != "complete")
            && ((d.type == "load" ? a : f).detachEvent("on" + d.type, b, !1), !e
            && (e = !0))) {
            c.call(a, d.type || d)
          }
        }
