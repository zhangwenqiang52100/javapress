...
;(function ($, window, document, undefined) {
  ...
  var Utils = {
    ...
    getLessVar: function (name, prop) {
      var value = $('<p class="' + name + '"></p>').hide().appendTo('body').css(
        prop)
      $('.' + name).remove()
    ...
    getIdealType: function ($el) {
      ...
      return (
        /(text|password|email|number|search|url|tel|textarea)/.test(type)
        && 'text' ||
        /file/.test(type) && 'file' ||
    ...
    makeInput: function (name, value, type, list, placeholder) {
      ...
      // Text & file
      if (/^(text|password|email|number|search|url|tel|file|hidden)$/.test(
          type)) {
        markup = '<input ' +
          'type="' + type + '" ' +
          'id="' + name + '" ' +
      ...
      if (/textarea/.test(type)) {
        markup = '<textarea id="' + name + '" name="' + name + '" value="'
          + value + '"></textarea>'
      }
    ...
    var
      ...
      Methods = {
        ...
        updateCounter: function (nameOrIdx, text) {
          var idx = !isNaN(nameOrIdx) ? nameOrIdx : Actions.getTabIdxByName(
            name),
        ...
        var
          ...
          items = (function () {
            ...
            $options.each(function () {
              var $this = $(this)
              items.push(
                '<li class="ideal-select-item">' + $this.text() + '</li>')
            })
            return items
          }())
      ...
      $this.on({
        change: function () {
          ...
          if ($this.is('input[type="radio"]')) {
            $this.parent().siblings('label').find('.ideal-radio').removeClass(
              'checked')
          }
    ...
    $.fn.idealFile = function () {
      ...
      return this.each(function () {
        ...
        var $file = $(this).addClass('ideal-file'), // the original file input
          ...
          $input = $(
            '<input type="text" class="ideal-file-filename" name="fileName"/>'),
          // Button that will be used in non-IE browsers
          $button = $(
            '<button type="button" class="ideal-file-upload">Open</button>'),
          // Hack for IE
          $label = $('<label class="ideal-file-upload" for="' + $file[0].id
            + '">Open</label>')
    ...
    var filters = {
      ...
      extension: {
        regex: function (input, value) {
          ...
          this.error = $.idealforms.errors.extension.replace('{0}',
            extensions.join('", "'))
          return valid
        }
      },
...
// Get LESS variables
  ...
  $.extend(IdealForms.prototype, {
    ...
    _getUserInputs: function () {
      return this.$form.find(
        '[name="' + Utils.getKeys(this.opts.inputs).join('"], [name="') + '"]')
    },
    ...
    _doMarkup: function ($element) {
      ...
      var idealElements = {
        ...
        radiocheck: function () {
          ...
          if (!isWrapped) {
            $element.parent().nextAll().andSelf().wrapAll(
              $field.addClass('ideal-radiocheck'))
            $element.parents('.ideal-field').append($valid, $invalid).after(
              $error)
          }
          ...
          if (theFilter) {
            ...
            // All other filters
              ...
              var valid = Utils.isRegex(theFilter.regex)
                && theFilter.regex.test(value) ||
                Utils.isFunction(theFilter.regex) && theFilter.regex(inputData,
                  value)
    ...
    _responsive: function () {
      ...
      this.opts.responsiveAt === 'auto'
        ...
        : this.$form.toggleClass('stack',
        $(window).width() < this.opts.responsiveAt)
  ...
  $.extend(IdealForms.prototype, {
    ...
    focusFirstInvalid: function () {
      var $first = this.getInvalid().first().find(
        'input:first, select, textarea')
      var tabName = $first.parents('.ideal-tabs-content').data(
        'ideal-tabs-content-name')
      if (this.$tabs.length) {
  ...
  $("#username").on("blur", function () {
    var $this = $(this);
    $.getJSON("/rest/v1/checkusername?username=" + $this.val(),
      function (date) {
        if (date == false) {
